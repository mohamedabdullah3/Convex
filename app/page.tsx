...
type ApiResponse = { report: AuditOutput; extraction: { fetched: boolean; limitation?: string } };

const defaults: AuditInput = { websiteUrl: '', country: '', industry: '', businessModel: 'Ecommerce', conversionGoal: 'Purchase', trafficSource: 'Google Search', audienceStage: 'Cold', ticketSize: 'Medium', language: 'English', businessNotes: '' };
const options = {
  ...
  trafficSource: ['Google Search', 'Meta Ads', 'TikTok Ads', 'Snapchat Ads', 'Organic', 'Email', 'Mixed'],
  audienceStage: ['Cold', 'Warm', 'Hot', 'Retargeting', 'Mixed'],
  ticketSize: ['Low', 'Medium', 'High', 'Premium'],
  language: ['Arabic', 'English', 'Bilingual']
} as const;

export default function Page() { const [form, setForm] = useState(defaults); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [data, setData] = useState<ApiResponse | null>(null);
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setError(null); const validated = auditInputSchema.safeParse(form); if (!validated.success) return setError('Please complete all required fields with a valid URL.'); setLoading(true); try { const res = await fetch('/api/audit', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(validated.data) }); const json = await res.json(); if (!res.ok) throw new Error(json.error || 'Request failed'); setData(json);} catch (err) { setError(err instanceof Error ? err.message : 'Unknown error'); } finally { setLoading(false);} };
